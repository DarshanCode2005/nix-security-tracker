...
def _get_or_create_cache(model_class: type[Model], field_name: str, values: set[str]) -> dict[str, Model]:
    """
...
class CveBulkContext:
    ...
    m2m_description_media: list[tuple[models.Description, models.SupportingMedia]] = field(default_factory=list)
    m2m_reference_tags: list[tuple[models.Reference, str]] = field(default_factory=list)
    m2m_problemtype_description: list[tuple[models.ProblemType, models.Description]] = field(default_factory=list)
    m2m_problemtype_references: list[tuple[models.ProblemType, models.Reference]] = field(default_factory=list)
    m2m_metric_scenarios: list[tuple[models.Metric, models.Description]] = field(default_factory=list)
    
    m2m_affected_platforms: list[tuple[models.AffectedProduct, str]] = field(default_factory=list)
    m2m_affected_versions: list[tuple[models.AffectedProduct, models.Version]] = field(default_factory=list)
    m2m_affected_cpes: list[tuple[models.AffectedProduct, str]] = field(default_factory=list)
    m2m_affected_modules: list[tuple[models.AffectedProduct, str]] = field(default_factory=list)
    m2m_affected_program_files: list[tuple[models.AffectedProduct, str]] = field(default_factory=list)
    m2m_affected_program_routines: list[tuple[models.AffectedProduct, str]] = field(default_factory=list)

    m2m_container_descriptions: list[tuple[models.Container, models.Description]] = field(default_factory=list)
    m2m_container_affected: list[tuple[models.Container, models.AffectedProduct]] = field(default_factory=list)
    m2m_container_problem_types: list[tuple[models.Container, models.ProblemType]] = field(default_factory=list)
    m2m_container_references: list[tuple[models.Container, models.Reference]] = field(default_factory=list)
    m2m_container_metrics: list[tuple[models.Container, models.Metric]] = field(default_factory=list)
    m2m_container_configurations: list[tuple[models.Container, models.Description]] = field(default_factory=list)
    m2m_container_workarounds: list[tuple[models.Container, models.Description]] = field(default_factory=list)
    m2m_container_solutions: list[tuple[models.Container, models.Description]] = field(default_factory=list)
    m2m_container_exploits: list[tuple[models.Container, models.Description]] = field(default_factory=list)
    m2m_container_timeline: list[tuple[models.Container, models.Event]] = field(default_factory=list)
    m2m_container_tags: list[tuple[models.Container, str]] = field(default_factory=list)
    m2m_container_credits: list[tuple[models.Container, models.Credit]] = field(default_factory=list)


def prepare_organization(uuid: str | None, ctx: CveBulkContext, short_name: str | None = None) -> models.Organization | None:
    if uuid is None:
        return None
...
def prepare_description(data: dict[str, Any], ctx: CveBulkContext) -> models.Description:
    obj = models.Description(
        lang=data["lang"],
        value=data["value"]
    )
...
def prepare_problem_type(data: dict[str, Any], ctx: CveBulkContext) -> models.ProblemType:
    obj = models.ProblemType(
        cwe_id=data.get("cweId"),
        _type=data.get("type")
    )
    ctx.problem_types.append(obj)
    
    desc = prepare_description({"lang": data["lang"], "value": data["description"]}, ctx)
...
def prepare_affected_product(data: dict[str, Any], ctx: CveBulkContext) -> models.AffectedProduct:
...
def prepare_cve_record(data: dict[str, Any], ctx: CveBulkContext, cve: models.CveRecord | None = None) -> models.CveRecord:
    if cve is None:
    ...
    org = prepare_organization(
        uuid=data["assignerOrgId"], 
        ctx=ctx,
        short_name=data.get("assignerShortName")
    )
...
def prepare_container(data: dict[str, Any], _type: str, cve: models.CveRecord, ctx: CveBulkContext) -> models.Container:
...
def prepare_cve(data: dict[str, Any], ctx: CveBulkContext, record: models.CveRecord | None = None, triaged: bool = False) -> models.CveRecord:
    ...
    prepare_container(data["containers"]["cna"], _type=models.Container.Type.CNA, cve=cve, ctx=ctx)
...
def _bulk_m2m(through_model: type[Model], tuples: list[tuple[Model, Model]], source_field: str, target_field: str):
    """
    ...
    objs = [
        through_model(**{
            f"{source_field}_id": source.pk,
            f"{target_field}_id": target.pk
        })
...
def _bulk_m2m_str(through_model: type[Model], tuples: list[tuple[Model, str]], source_field: str, target_field: str, cache: dict[str, Model]):
    """
    ...
    objs = [
        through_model(**{
            f"{source_field}_id": source.pk,
            f"{target_field}_id": cache[target].pk
        })
        for source, target in tuples
    ]
    through_model.objects.bulk_create(objs)
...
def flush(ctx: CveBulkContext):
    ...
    with pgtrigger.ignore(
        ...
        if ctx.orgs:
            models.Organization.objects.bulk_create(ctx.orgs.values(), ignore_conflicts=True)
            
        ctx.tags = _get_or_create_cache(models.Tag, "value", ctx.tags_required)
        ctx.platforms = _get_or_create_cache(models.Platform, "name", ctx.platforms_required)
        ...
        ctx.program_files = _get_or_create_cache(models.ProgramFile, "name", ctx.program_files_required)
        ctx.program_routines = _get_or_create_cache(models.ProgramRoutine, "name", ctx.program_routines_required)
        ...
        _bulk_m2m(models.Description.media.through, ctx.m2m_description_media, "description", "supportingmedia")
        
        _bulk_m2m_str(models.Reference.tags.through, ctx.m2m_reference_tags, "reference", "tag", ctx.tags)
        
        _bulk_m2m(models.ProblemType.description.through, ctx.m2m_problemtype_description, "problemtype", "description")
        _bulk_m2m(models.ProblemType.references.through, ctx.m2m_problemtype_references, "problemtype", "reference")
        
        _bulk_m2m(models.Metric.scenarios.through, ctx.m2m_metric_scenarios, "metric", "description")
        
        _bulk_m2m_str(models.AffectedProduct.platforms.through, ctx.m2m_affected_platforms, "affectedproduct", "platform", ctx.platforms)
        _bulk_m2m(models.AffectedProduct.versions.through, ctx.m2m_affected_versions, "affectedproduct", "version")
        _bulk_m2m_str(models.AffectedProduct.cpes.through, ctx.m2m_affected_cpes, "affectedproduct", "cpe", ctx.cpes)
        _bulk_m2m_str(models.AffectedProduct.modules.through, ctx.m2m_affected_modules, "affectedproduct", "module", ctx.modules)
        _bulk_m2m_str(models.AffectedProduct.program_files.through, ctx.m2m_affected_program_files, "affectedproduct", "programfile", ctx.program_files)
        _bulk_m2m_str(models.AffectedProduct.program_routines.through, ctx.m2m_affected_program_routines, "affectedproduct", "programroutine", ctx.program_routines)
        
        _bulk_m2m(models.Container.descriptions.through, ctx.m2m_container_descriptions, "container", "description")
        _bulk_m2m(models.Container.affected.through, ctx.m2m_container_affected, "container", "affectedproduct")
        _bulk_m2m(models.Container.problem_types.through, ctx.m2m_container_problem_types, "container", "problemtype")
        _bulk_m2m(models.Container.references.through, ctx.m2m_container_references, "container", "reference")
        _bulk_m2m(models.Container.metrics.through, ctx.m2m_container_metrics, "container", "metric")
        _bulk_m2m(models.Container.configurations.through, ctx.m2m_container_configurations, "container", "description")
        _bulk_m2m(models.Container.workarounds.through, ctx.m2m_container_workarounds, "container", "description")
        _bulk_m2m(models.Container.solutions.through, ctx.m2m_container_solutions, "container", "description")
        _bulk_m2m(models.Container.exploits.through, ctx.m2m_container_exploits, "container", "description")
        _bulk_m2m(models.Container.timeline.through, ctx.m2m_container_timeline, "container", "event")
        _bulk_m2m_str(models.Container.tags.through, ctx.m2m_container_tags, "container", "tag", ctx.tags)
        _bulk_m2m(models.Container.credits.through, ctx.m2m_container_credits, "container", "credit")
